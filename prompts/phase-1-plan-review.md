...
You are reviewing a coding plan before any code is written. The plan is at `plans/raw-plan.md` or `plans/reviewed-plan.md`. The current state of the agent loop is in `/tmp/agent-loop-state.json`.
